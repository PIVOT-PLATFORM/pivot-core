...
/**
 * EN53.1 — fait cohabiter le schéma {@code public} (shell) et le schéma {@code agilite} (module
 * absorbé) comme deux migrations Flyway distinctes dans le même contexte Spring de l'app agrégée.
 *
 ...
 * Déclarer un <em>bean</em> {@link Flyway} supplémentaire pour {@code agilite} <strong>supprime</strong>
 * donc entièrement le bean public de Boot — le schéma {@code public} n'est alors jamais migré, et la
 * migration {@code agilite} (dont la FK pointe vers {@code public.tenants}) échoue avec
 * {@code relation "public.tenants" does not exist} (constaté en test local, EN53.1).
 *
 ...
 * l'absorption d'agilite. La migration {@code agilite} est déclenchée par l'{@link ApplicationRunner}
 * ci-dessous, <strong>après</strong> le refresh (donc après que {@code public.tenants}/{@code teams}
 * existent) — sûr car {@code spring.jpa.hibernate.ddl-auto=none} dans l'app agrégée (aucune validation
 * de schéma à l'init de l'EMF). Le Flyway d'agilite est construit <em>inline</em> (jamais exposé en
 * bean) précisément pour ne pas retomber dans la suppression du bean public de Boot.
 *
 * <p>Les contextes de test isolés du module agilite ({@code AgiliteTestApplication}) n'ont pas cette
 * classe (elle est sous {@code fr.pivot.config}, shell) et migrent {@code agilite} via leur propre
 * {@code spring.flyway.locations} — ce config ne concerne que l'app agrégée.
 */
@Configuration
