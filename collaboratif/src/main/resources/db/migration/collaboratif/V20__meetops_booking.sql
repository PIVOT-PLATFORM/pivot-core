...
-- agenda_items) and V18 (US12.2.1's animation lifecycle values) — never touches V1..V19.
--
...
-- meetings: widen chk_meeting_status again (V17 restricted it to 'DRAFT' only; V18 widened it to
-- also accept CONFIRMED/IN_PROGRESS/ENDED for the animation lifecycle) to also accept
-- PRE_RESERVED (US12.4.1's booking-flow status, MeetingStatus enum) — every prior value stays
-- accepted, US12.1.1/US12.2.1's flows are unaffected. booking_window_*/event_ref/project_ref are
-- the correlation fields for a meeting created from a roadmap.event.window.* event; event_ref/
...
ALTER TABLE collaboratif.meetings
    ADD CONSTRAINT chk_meeting_status
        CHECK (status IN ('DRAFT', 'PRE_RESERVED', 'CONFIRMED', 'IN_PROGRESS', 'ENDED'));
