package fr.pivot.collaboratif.testsupport.auth;

import fr.pivot.collaboratif.testsupport.auth.entity.PlatformAccessToken;
...
import fr.pivot.collaboratif.testsupport.auth.repository.AccessTokenReadRepository;
import fr.pivot.collaboratif.testsupport.auth.repository.PlatformTenantReadRepository;
import fr.pivot.collaboratif.testsupport.auth.repository.PlatformUserReadRepository;
 ...
 * genuinely standalone deployment, never a network call back to {@code pivot-core}), alongside a
 * {@code fr.pivot.collaboratif.config.SecurityConfig} that permitted every request through Spring
 * Security unconditionally (a workaround required only because {@code pivot-core-starter} 0.27.1
 * transitively pulled in {@code spring-boot-starter-security} onto this module's classpath). Now
 * that this module is aggregated into {@code pivot-core-app}'s single JVM/Spring context, both of
 * those have been removed entirely: {@code fr.pivot.auth.service.TokenService} (the shell's own
 * opaque-token service) is the sole production {@link AuthenticatedPrincipalResolver} bean, and
 * the shell's own {@code fr.pivot.config.SecurityConfig} owns the aggregated app's security filter
 * chain — every consumer in this module ({@code
 * fr.pivot.collaboratif.context.CollaboratifRequestPrincipalResolver}, {@code
 * StompAuthenticationChannelInterceptor}) is wired against the shared {@link
 * AuthenticatedPrincipalResolver} interface, never against a concrete implementation, so it now
 * transparently authenticates through the shell's real token validation.
 *
 * <p>This class only backs this module's own <em>isolated</em> {@code @SpringBootTest} suite
 * ({@code fr.pivot.collaboratif.CollaboratifTestApplication}, {@code
 * @ComponentScan("fr.pivot.collaboratif")}): that context has no dependency on {@code
 * pivot-core-app}'s classes at all (this Maven module only depends on {@code
 * pivot-core-starter}), so without a local {@link AuthenticatedPrincipalResolver} bean, {@code
 * CollaboratifRequestPrincipalResolver}'s constructor injection would fail at context startup with
 * no candidate bean. Placed under {@code src/test/java} (never compiled into the production
 * library jar {@code pivot-core-app} depends on) so it can never be mistaken for, or collide with,
 * the shell's real bean once aggregated.
 *
 ...
 * equivalent so every existing {@code *IT} test in this module (including {@link
 * fr.pivot.collaboratif.auth.AuthenticationIT}'s cross-tenant/lifecycle assertions) keeps testing
 * the exact same security property, unaffected by which concrete bean happens to sit behind {@link
 * AuthenticatedPrincipalResolver} in a given runtime.
 *
 ...
 * CollaboratifRequestPrincipalResolver}) maps that uniformly to a generic 401 with no
 * discriminating detail, so a caller can never distinguish these cases from the HTTP response.
 */
@Service
